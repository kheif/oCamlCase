...
var MODULE_HINT_TYPES = {
    ...
    'Printf': {
        ...
        'ikfprintf': '(out_channel -> \'b) -> out_channel -> (\'a, out_channel, \'b) format -> \'a',
        'ksprintf': '(string -> \'b) -> (\'a, unit, string, \'b) format4 -> \'a',
        'kbprintf': '(Buffer.t -> \'b) -> Buffer.t -> (\'a, Buffer.t, \'b) format -> \'a',
        'kprintf': '(string -> \'b) -> (\'a, unit, string, \'b) format4 -> \'a',
    },
    'List': {
        'length': '\'a list -> int',
        'hd': '\'a list -> \'a',
    ...
    'Option': {
        ...
        'to_result': 'none:\'e -> \'a option -> (\'a, \'e) result',
...
function renderHintItem(el, data, cur) {
    ...
    nameSpan.className = 'cm-hint-name';
    nameSpan.textContent = cur.text;
    el.appendChild(nameSpan);
    if (cur.typeStr) {
        var typeSpan = document.createElement('span');
        typeSpan.className = 'cm-hint-type';
        typeSpan.textContent = cur.typeStr;
        el.appendChild(typeSpan);
    }
}

function makeHintItem(text, module) {
    var typeStr = module && MODULE_HINT_TYPES[module] && MODULE_HINT_TYPES[module][text]
        ? MODULE_HINT_TYPES[module][text]
        : null;
    if (!typeStr) return text;
    return { text: text, render: renderHintItem, typeStr: typeStr };
}
...
function hint_prediction(cm, option) {
    return new Promise(function (accept) {
        setTimeout(function () {
            // Get the word before the cursor position (100ms debounce)
            ...
            if (/\./.test(line.charAt(start - 1))) { // Special module case
                ...
                if (MODULE_HINT.hasOwnProperty(module)) {
                    if (word.length == 0) {
                        return accept({
                            list: MODULE_HINT[module].map(function(t) { return makeHintItem(t, module); }),
                            from: CodeMirror.Pos(cursor.line, start),
                            to: CodeMirror.Pos(cursor.line, end)
                        })
                    } else if (!MODULE_HINT[module].includes(word)) {
                        return accept({
                            list: includer(MODULE_HINT[module], word).map(function(t) { return makeHintItem(t, module); }),
                            from: CodeMirror.Pos(cursor.line, start),
                            to: CodeMirror.Pos(cursor.line, end)
                        })
                    }
            ...
            if (word.length !== 0 && correspondance.length !== 0 && !possibilities.includes(word)) {
                return accept({
                    list: correspondance.slice(0, 5).map(function(t) { return makeHintItem(t, 'Base'); }),
                    from: CodeMirror.Pos(cursor.line, start),
                    to: CodeMirror.Pos(cursor.line, end)
                })
            }
...
function create_editor(id, name) {
    ...
    editor.on("keyup", function (cm, event) {
        if (cm.ext_autocomplete && // Only trigger if jetbrain style autocompletion is activated
            cm.autocomplete_timeout < Date.now() - 250 && // Only trigger if the user stopped typing
