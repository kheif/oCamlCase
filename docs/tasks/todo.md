...
## Guided Path / Reference dual-mode restructure ✅ DONE (2026-06-13)

Source: root TODO.md (pedagogical ordering problem). Plan: `~/.claude/plans/you-are-a-senior-typed-pumpkin.md`.

- [x] Ordering centralized: `src/content/order.ts` (guidedPhases + pagePosition + drift
  invariant that throws at module load); registry array order = reference order, all
  numbering computed (nav.num now optional, concept nums derived)
- [x] `ContentPageDef.concept` field (tocDesc, footerExtras, unnumbered); page-label
  "Concept N of 27" + "Continue learning" footers stripped from all concept HTMLs and
  portal-injected by ContentPage from registry + mode (hosts created post
  dangerouslySetInnerHTML, `.page-header > .page-label` stagger preserved)
- [x] Mode infra: LearnModeProvider (localStorage `learn-mode`, default reference) +
  useLearnMode in `src/learn/`; ModeToggle pill pair on home (mirrors mini-view-toggle);
  nav.ts → getNavGroups(mode)/getTopbarLinks(mode); Sidebar/Topbar/MiniExercisePage wired
- [x] home.html → `src/features/home/Home.tsx` (same class names, counts computed from
  registry/miniExercises); guided TOC groups by phase with .toc-phase-head
- [x] Animation: `src/hooks/useFlip.ts` FLIP on sidebar links + home TOC items
  (key-based snapshots survive parent-group remounts; reduced-motion skips);
  label/footer .mode-crossfade 150ms
- [x] Split constructors-exceptions → `variants.html` + `exceptions.html` (styles split
  per demo); redirects in App.tsx; inbound links fixed (options-result, match-shape.ts)
- [x] 7 new pages: how-to-learn (unnumbered, Phase 0), compiler-errors, tuples,
  modules, functors, projects-dune-opam, testing-and-quality
- [x] Guided order: P0 orientation / P1 bindings / P2 abstractions→currying /
  P3 types+errors / P4 tuples→variants→pattern-matching→options→records→exceptions /
  P5 lists→sorting (capstone) / P6 mutability→modules→functors→dune→testing
- [x] Sweep: sitemap 45 URLs, index.html meta+JSON-LD 27 concepts, README learning path,
  ExercisesHub/MiniExerciseIndex stale counts, variants category label
- [x] verify: build + lint clean (2 pre-existing no-empty untouched); vite-node check:
  both prev/next chains intact end-to-end, 27 numbered + orientation, currying =
